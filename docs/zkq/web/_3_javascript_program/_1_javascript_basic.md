...
|`str[index]`|获取指定位置处字符|`HTML5`，`IE8+` 支持，相当于 `charAt()`|
...
##### 14.6.5.1 拼接字符串：`concat`

|**方法名**|**说明**|
|:-|:-|
|`concat(str1, str2, str3, ...)`|`concat()` 方法用于连接两个或多个字符串。即拼接字符串，等效于 `+`，但 `+` 更常用|
...
##### 14.6.5.2 截取字符串：`substr` & `slice` & `substring`

|**方法名**|**说明**|
|:-|:-|
|`substr(start, count)`|从 `start` 位置开始（索引号），`count` 取的个数|
|`slice(start, end)`|从 `start` 位置开始，截取到 `end` 位置，`end` 取不到|
|`substring(start, end)`|从 `start` 位置开始，截取到 `end` 位置，`end` 取不到。（基本和 `slice` 相同，但是不接受负值）|

```js:no-line-numbers
var str1 = '改革春风吹满地';
console.log(str1.substr(2, 2)); // 第一个2是索引号的2，即从第几个开始；第二个2是取几个字符
```

##### 14.6.5.3 替换字符串：`replace`

|**方法名**|**说明**|
|:-|:-|
|`replace(被替换的字符串, 要替换为的字符串)`|用于在字符串中用一些字符替换另一些字符，只会替换第一个匹配的字符|
...
##### 14.6.5.4 分割字符串：`split`

|**方法名**|**说明**|
|:-|:-|
|`split(分割符)`|用于切分字符串，它可以将字符串切分为数组，返回的是一个新数组|
...
##### 14.6.5.5 转大写：`toUpperCase`

|**方法名**|**说明**|
|:-|:-|
|`toUpperCase()`|转换大写|

##### 14.6.5.6 转小写：`toLowerCase`

|**方法名**|**说明**|
|:-|:-|
|`toLowerCase()`|转换小写|

## 15. `JavaScript` 简单类型（基本数据类型）& 复杂类型（引用类型）

### 15.1 概述

#### 15.1.1 简单类型（又称基本数据类型，或值类型）
...
![](./images/_1_javascript_basic/16.png)
...
![](./images/_1_javascript_basic/17.png)
